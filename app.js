...
const STORAGE_KEY = "comptaquest_state_v1";
...
const ACCESS_CODES = ["COMPTA-2026-SJO5"]; // code(s) fourni(s) au centre de formation après achat de la licence
...
const SYNC_KEY = "comptaquest_sync_pending";
...
function deviceId() {
  let id = localStorage.getItem("comptaquest_device_id");
  if (!id) {
    id = "cq-" + (crypto.randomUUID ? crypto.randomUUID() : Date.now().toString(36) + Math.random().toString(36).slice(2));
    localStorage.setItem("comptaquest_device_id", id);
  }
  return id;
}
